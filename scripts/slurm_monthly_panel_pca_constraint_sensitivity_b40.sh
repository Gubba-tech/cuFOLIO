#!/usr/bin/env bash
#SBATCH --job-name=cufolio-s16-constraints-b40
...
#SBATCH --exclusive
#SBATCH --output=artifacts/paper_replay/gpu_logs/monthly-panel-pca-constraints-b40-%j.log
...
end_epoch="$(date +%s)"
cat > "$log_dir/monthly-panel-pca-constraints-b40-${SLURM_JOB_ID:-local}.metadata" <<EOF
