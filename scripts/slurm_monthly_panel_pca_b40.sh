#!/usr/bin/env bash
#SBATCH --job-name=cufolio-s15-pca-b40
#SBATCH --partition=b40x4
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=32
#SBATCH --mem=128G
#SBATCH --time=08:00:00
#SBATCH --output=artifacts/paper_replay/gpu_logs/monthly-panel-b40-%j.log
...
run_dir="artifacts/paper_replay/results/monthly_panel_pca_k6_2020_2022_240m_cuopt_b40"
mkdir -p "$log_dir"
start_epoch="$(date +%s)"
status=0
...
set +e
{
    ...
    if [[ "${RUN_SHORT_LOOKBACK:-0}" == "1" ]]; then
        uv run python scripts/run_monthly_panel_pca_pilot.py \
            --monthly-panel /lustre/nvwulf/home/weicdeng/PortOpt-IPCA-GPU/data/dfall_for_test.csv \
            --output-dir artifacts/paper_replay/results/monthly_panel_pca_k6_2005_2022_60m_cuopt_b40 \
...
} 2>&1 | tee "$log_dir/monthly-panel-b40-${SLURM_JOB_ID:-local}.log"
status=${PIPESTATUS[0]}
set -e

end_epoch="$(date +%s)"
cat > "$log_dir/monthly-panel-b40-${SLURM_JOB_ID:-local}.metadata" <<EOF
